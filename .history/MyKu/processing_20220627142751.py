...
HASOC2020数据集加载函数
"""
def set_hasoc2020_train_data(hasoc_2020):
    training_data = pd.read_csv(ORIGIN_DATASET_PATH + 'hasoc2020/' + hasoc_2020)
    for index in range(len(training_data)):
        text = training_data.iloc[index]['text']
        text = Pre_processing_tweets().clean_unuseful(text)
        training_data.iloc[index]['text'] = text
    training_data.to_csv(HASOC_2020 + '/' + hasoc_2020, index=False, sep='\t')
...
def create_hasoc2020():
    if os.path.exists(HASOC_2020):
        return
    os.mkdir(HASOC_2020)
    set_exist2021_train_data('hasoc_2020_en_train.xlsx')
    set_exist2021_test_data(origin_exist_2021)
