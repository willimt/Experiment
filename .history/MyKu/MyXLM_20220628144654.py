...
class MyXlmModel(nn.Module):
    ...
    def __init__(self, class_size, pretrained_name='xlm-roberta-base'):
        ...
        self.xlm = XLMModel.from_pretrained(pretrained_name,
                                              return_dict=True)
    ...
    def forward(self, inputs):
        ...
        input_ids, input_tyi, input_attn_mask = inputs['input_ids'], inputs['attention_mask']
        # 将三者输入进模型，如果想知道模型内部如何运作，前面的蛆以后再来探索吧~
        output = self.xlm(input_ids, input_tyi, input_attn_mask)
        # bert_output 分为两个部分：
