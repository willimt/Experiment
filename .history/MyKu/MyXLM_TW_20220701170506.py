...
class MyXlmModel(nn.Module):

    # 初始化类
    def __init__(self, class_size, pretrained_name='cardiffnlp/twitter-xlm-roberta-base-sentiment'):
        """
        Args:
        ...
        """
        # 类继承的初始化，固定写法
        super(MyXlmModel, self).__init__()
        # 加载HuggingFace的BertModel
        # XlmModel的最终输出维度默认为768
        # return_dict=True 可以使XlmModel的输出具有dict属性，即以 xlm_output['last_hidden_state'] 方式调用
        self.xlm = XLMRobertaForSequenceClassification.from_pretrained(
            pretrained_name, return_dict=True)
        # 通过一个线性层将[CLS]标签对应的维度：768->class_size
        # class_size 在SST-2情感分类任务中设置为：2
        self.xlm.classifier.out_proj = nn.Linear(768, class_size)
    ...
    def forward(self, inputs):
        ...
        input_ids, input_attn_mask = inputs['input_ids'], inputs['attention_mask']
        # 将三者输入进模型，如果想知道模型内部如何运作，前面的蛆以后再来探索吧~
        output = self.xlm(input_ids, input_attn_mask)
        # xlm_output 分为两个部分：
...
tokenizer = XLMRobertaTokenizer.from_pretrained(pretrained_model_name)
...
def load_xlm_data(train_data, test_data, batch_size):
    train_dataset = XLMDataset(train_data)
    test_dataset = XLMDataset(test_data)
    train_dataloader = DataLoader(
        train_dataset, batch_size=batch_size, collate_fn=coffate_fn, shuffle=True)
    test_dataloader = DataLoader(
        test_dataset, batch_size=1, collate_fn=coffate_fn)
...
def test(model, test_dataloader, device, epoch, file_name):
    ...
    print(f'epochs : {epoch}\n', metrics.confusion_matrix(
        true_y, pred_y), file=fp)
    print(f'epochs : {epoch}\n', metrics.classification_report(
        true_y, pred_y), file=fp)
    print(metrics.classification_report(true_y, pred_y, average="weighted"))
    acc = metrics.accuracy_score(true_y, pred_y, average="weighted")
    print(f'Acc : {acc}\t F1: {metrics.f1_score(true_y, pred_y, average="weighted")}')
    print(f'Acc : {acc}\t F1: {metrics.f1_score(true_y, pred_y, average="weighted")}\n', file=fp)
    fp.close()
    return acc
