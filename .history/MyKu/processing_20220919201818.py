...
def get_OLID_train_data(data_dir):
    res = []
    data = pd.read_csv(data_dir, sep='\t')
    lens = len(data)
    for index in range(lens):
        ...
        if item['tweet'] != item['tweet']:
            continue
        text = item['tweet']
        label = 1 if item['subtask_a'] == 'OFF' else 0
        res.append((text, label))
    return res


def get_OLID_testA_data(text_dir):
    res = []
    texts = pd.read_csv(text_dir, sep='\t')
    lens = len(texts)
    for index in range(lens):
...
def set_sem2018_train_data(origin_sem):
    training_data = pd.read_csv(origin_sem + '/train_taskA.tsv', sep='\t')
    for index in range(len(training_data)):
        text = training_data.iloc[index]['Tweet text']
        text = Pre_processing_tweets().tokenize_process(text)
        training_data.iloc[index, 2] = text
...
def set_sem2018_testA_data(origin_sem):
    testing_data = pd.read_csv(origin_sem + '/test_taskA.tsv', sep='\t')
    for index in range(len(testing_data)):
        text = testing_data.iloc[index]['Tweet text']
        text = Pre_processing_tweets().tokenize_process(text)
        testing_data.iloc[index, 2] = text
        # training_data.iloc[index,1] = 1 if training_data.iloc[index]['subtask_a'] == 'OFF' else 0
    testing_data.to_csv(SEM2018_DATASET + '/train.tsv', index=False, sep='\t')


def get_SEM_train_data(data_dir):
    res = []
    data = pd.read_csv(data_dir, sep='\t')
    lens = len(data)
    for index in range(lens):
        ...
        label = 1 if item['Label'] == 'OFF' else 0
        res.append((text, label))
    return res


def get_SEM_testA_data(text_dir):
    res = []
    texts = pd.read_csv(text_dir, sep='\t')
    lens = len(texts)
    for index in range(lens):
