...
class MyXlmModel(nn.Module):
    ...
    # 初始化类
    def __init__(self, class_size, pretrained_name='xlm-roberta-base'):
        """
        Args:
        ...
        config = XLMRobertaConfig.from_pretrained(pretrained_name)
        config.output_attentions = True
        self.xlm = XLMRobertaModel.from_pretrained(
            pretrained_name, return_dict=True, config = config)
...
def train(model, train_dataloader, device, optimizer, loss, epoch):
    ...
    for batch in tqdm(train_dataloader, desc=f"Training Epoch {epoch}", colour='red'):
        ...
        bert_output = model.classifier(model(inputs).pooler_output)
...
def test(model, test_dataloader, device, epoch, file_name):
    ...
    print(f'epochs : {epoch}\n', metrics.confusion_matrix(true_y, pred_y), file = fp)
