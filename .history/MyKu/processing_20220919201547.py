...
def set_sem2018_train_data(origin_sem):
    training_data = pd.read_csv(origin_sem + '/.tsv', sep='\t')
    for index in range(len(training_data)):
    ...
    training_data.to_csv(SEM + '/train.tsv', index=False, sep='\t')


def set_sem2018_testA_data(origin_sem):
    testing_data = pd.read_csv(origin_sem + '/testset-levela.tsv', sep='\t')
    label_data = pd.read_csv(origin_sem + '/labels-levela.csv')
    testing_data['label'] = ''
    for index in range(len(testing_data)):
        # text = testing_data.iloc[index]['tweet']
        label = 1 if label_data.iloc[index]['label'] == 'OFF' else 0
